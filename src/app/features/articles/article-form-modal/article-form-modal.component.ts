// article-form-modal.component.ts
...
export interface ArticleFormData {
  article?: Article;  // undefined = création
}
...
@Component({
  ...
  imports: [
    ...
    MatInputModule, MatTooltipModule, ReactiveFormsModule, CommonModule,
  ],
  template: `
            ...
            <div class="input-group input-group-sm">
              ...
              @if (!isEdit) {
                <button class="btn btn-outline-secondary" type="button"
                        (click)="genCode()" matTooltip="Générer un code">
          ...
          <div class="col-12">
            <label class="form-label small fw-semibold">
              ...
              <i class="fa-solid fa-eye-slash text-muted ms-1"
                 matTooltip="Visible Admin uniquement"></i>
            </label>
...
export class ArticleFormModalComponent {
  ...
  async sauvegarder(): Promise<void> {
    ...
    // Conversions numériques (les inputs retournent des strings)
    ...
    if (this.isEdit) {
      await this.data$.updateArticle(val);
    } else {
      await this.data$.addArticle(val);
    }
